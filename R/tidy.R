...
ebc_tidy <- function(
  ...
  if ("TP" %in% measures) {
    df$TP <- ebc_TP(detected, true)
  }
  if ("FP" %in% measures) {
    df$FP <- ebc_FP(detected, true)
  }
  if ("FN" %in% measures) {
    df$FN <- ebc_FN(detected, true)
  }
  if ("TN" %in% measures) {
    df$TN <- ebc_TN(detected, true, m = m)
  }
  if ("TPR" %in% measures) {
    df$TPR <- ebc_TPR(detected, true)
  }
  if ("TNR" %in% measures) {
    df$TNR <- ebc_TNR(detected, true, m = m)
  }
  if ("PPV" %in% measures) {
    df$PPV <- ebc_PPV(detected, true)
  }
  if ("NPV" %in% measures) {
    df$NPV <- ebc_NPV(detected, true, m = m)
  }
  if ("FNR" %in% measures) {
    df$FNR <- ebc_FNR(detected, true)
  }
  if ("FPR" %in% measures) {
    df$FPR <- ebc_FPR(detected, true, m = m)
  }
  if ("FDR" %in% measures) {
    df$FDR <- ebc_FDR(detected, true)
  }
  if ("FOR" %in% measures) {
    df$FOR <- ebc_FOR(detected, true, m = m)
  }
  if ("ACC" %in% measures) {
    df$ACC <- ebc_ACC(detected, true, m = m)
  }
  if ("BACC" %in% measures) {
    df$BACC <- ebc_BACC(detected, true, m = m)
  }
  if ("F1" %in% measures) {
    df$F1 <- ebc_F1(detected, true)
  }
  if ("PLR" %in% measures) {
    df$PLR <- ebc_PLR(detected, true, m = m)
  }
  if ("NLR" %in% measures) {
    df$NLR <- ebc_NLR(detected, true, m = m)
  }
  if ("DOR" %in% measures) {
    df$DOR <- ebc_DOR(detected, true, m = m)
  }
