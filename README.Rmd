...
The goal of **evabic** is to **eva**luate **bi**nary **c**lassifiers by specifying what is detected as true and what is actually true. It has no dependancies.
...
__evabic__ provides `r length(evabic::ebc_allmeasures)` handy functions to computes different measures. Each function begins with `ebc_*`.

Available measures includes True Positive Rate (Sensitivity or Recall), True Negative Rate (Specificity), Positive Predictive Value (Precision), False Discovery Rate, Accuracy, F1... and can be accessed with `ebc_allmeasures`.
...
Consider `X1`, `X2` and `X3`, `Y` a variable predicted by this three variables, and 4 more conditionally independent variables. 
